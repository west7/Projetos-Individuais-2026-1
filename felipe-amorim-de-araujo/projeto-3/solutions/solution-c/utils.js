...
// Strict schema validation: all fields must be present and semantically correct.
// type and severity must be recognized values (not 'unknown' — that signals AI failure).
// component=unknown is valid per the classification rules.
function isValidSchema(obj) {
  if (!obj || typeof obj !== 'object') return false;
  if (obj.parse_error) return false;
  if (!VALID_TYPES.includes(obj.type)) return false;
  if (!VALID_SEVERITIES.includes(obj.severity)) return false;
  if (!VALID_COMPONENTS.includes(obj.component)) return false;
  if (typeof obj.confidence !== 'number' || obj.confidence < 0 || obj.confidence > 1) return false;
  if (typeof obj.low_confidence !== 'boolean') return false;
  if (typeof obj.summary !== 'string' || obj.summary.trim().length === 0) return false;
  if (typeof obj.reasoning !== 'string') return false;
  return true;
}

// Default classification used when all Gemini attempts are exhausted.
function buildFallbackClassification(reason) {
  return {
    type: 'unknown',
    severity: 'unknown',
    component: 'unknown',
    ...
    reasoning: reason || 'Both Gemini API attempts returned invalid or unparseable output',
  };
}

// Attempt 1 → if schema invalid → attempt 2 (if provided) → fallback.
// Returns { classification, used_fallback, attempt }.
function processWithRetry(primaryText, retryText) {
  const primary = parseGeminiResponse(primaryText);
  if (isValidSchema(primary)) {
    return { classification: primary, used_fallback: false, attempt: 1 };
  }

  if (retryText !== undefined && retryText !== null) {
    const retry = parseGeminiResponse(retryText);
    if (isValidSchema(retry)) {
      return { classification: retry, used_fallback: false, attempt: 2 };
    }
  }
  ...
  return {
    ...
    used_fallback: true,
    attempt: (retryText !== undefined && retryText !== null) ? 2 : 1,
  };
}
...
function formatSlackMessage(issue, classification, aiFlagged = false) {
  ...
  return {
    ...
    blocks: [
      ...
      },
      {
        ...
        text: {
          type: 'mrkdwn',
          text: `*Summary:* ${classification.summary}${footer}`,
        },
      },
    ],
  };
...
// aiFlagged=true signals Gemini API failure after retry (not just low confidence).
function formatSheetsRow(issue, classification, processedAt, aiFlagged = false) {
