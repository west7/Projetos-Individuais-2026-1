...
function selectExamples(issue, knowledgeBase, k = 3) {
  if (!Array.isArray(knowledgeBase) || knowledgeBase.length === 0 || k === 0) {
    return [];
  }

  const issueLabels = new Set(normalizeLabels(issue && issue.labels));
  const issueTitleTokens = tokenize(issue && issue.title);
  const issueBodyTokens = tokenize(issue && issue.body);

  const scored = knowledgeBase.map((entry, idx) => {
    const exLabels = new Set(normalizeLabels(entry.issue && entry.issue.labels));
    const exTitleTokens = tokenize(entry.issue && entry.issue.title);
    const exBodyTokens = tokenize(entry.issue && entry.issue.body);

    const score =
      LABEL_WEIGHT * intersectionSize(issueLabels, exLabels) +
      TITLE_WEIGHT * intersectionSize(issueTitleTokens, exTitleTokens) +
      BODY_WEIGHT * intersectionSize(issueBodyTokens, exBodyTokens);
  ...
  if (scored[0].score === 0) {
    return knowledgeBase.slice(0, k);
  }
...
function buildPrompt(issue, examples = []) {
  ...
  return `You are an issue classifier for a software project. Analyze the GitHub issue below and return a JSON classification.
